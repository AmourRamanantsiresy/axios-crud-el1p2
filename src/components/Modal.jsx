...
import { InputLabel, InputGroupLabel } from "./List/InputsForms";
...
export default function Modal(props) {
    ...
    const { status, changeStatus, values, setEmployee, initialVal, loading } = props;
    ...
    const sendValues = () => {
        loading(e => !e)
        if (status.state === 1) {
            addUser(setEmployee, inputValues, initialVal, closeModal, loading);
        } else if (status.state === 2) {
            updateUser(setEmployee, inputValues, initialVal, closeModal, loading);
        }
    }
